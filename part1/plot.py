...
def plot_scores_and_times(pts_set):
    ...
    for i, (method, colour) in enumerate(
        zip(["random", "strat", "lhs"], ["red", "deepskyblue", "black"])
    ):
        for j, (ML_model, line) in enumerate(
            zip(["RF", "GP", "NN"], ["solid", "dotted", "dashed"])
        ):
            ...
            ax_2.plot(
                np.log10(pts_set),
                np.log10(times[j][i] + gen_times[i]),
                linestyle=line,
                color=colour,
                label=ML_model + " " + method,
            )
