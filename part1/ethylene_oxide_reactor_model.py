"""
The following physical model of an ethylene oxide plug flow reactor.
...
M_r = [28.05, 32, 44.05, 44.01, 18.015]
...
class UnitOp:
    ...
    def __init__(self, inlet, fluxes):
        ...
        self.outlet_prop = (
            np.array(inlet.list_prop) * slope + intcp
        )  # initialize outlet as the inlet
    ...
    def model(self, dx, model_inlet_prop):
        ...
        if T < 530 and T > 350:
            ...
            Cp = np.sum((np.array(C_pT) * T + np.array(C_p0)) * np.array(v_mol)) / (
                totmol + 1e-5
            )
            ...
            prop_step_change = np.array(
                [
                    (deltaH1 * r1 + deltaH2 * r2 + self.fluxes[0] * 1e6)
                    / (Cp * totmol),
                    0,
                    -r1 - r2,
                    -0.5 * r1 - 3 * r2,
    ...
    def results(self):
        self.calc_outlet()
        return [
            (self.outlet_prop[0] - intcp[0]) / slope[0],
            *(self.outlet_prop[2:7] - intcp[2:7]) / slope[2:7],
        ]
